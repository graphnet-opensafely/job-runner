...
# TOOLS_IMAGE = "opensafely-job-runner-tools:latest"
IMAGE_PULL_POLICY = "IfNotPresent"
TOOLS_IMAGE = "ccbidevdsacr.azurecr.io/opensafely-job-runner-tools:latest"
...
def test_generate_cohort_with_JobAPI(monkeypatch):
    ...
    monkeypatch.setattr("jobrunner.executors.graphnet.config.GRAPHNET_K8S_JOB_RUNNER_IMAGE", TOOLS_IMAGE)
...
def test_job_env(monkeypatch):
    ...
    storage = []
    pod_labels = dict()
    create_k8s_job(job, namespace, image, command, args, env, storage, pod_labels, image_pull_policy=IMAGE_PULL_POLICY)
...
def test_job_network(monkeypatch):
    ...
    storage = []
    create_k8s_job(job_allowed, namespace, image, command, args, {}, storage, github_network_labels, image_pull_policy=IMAGE_PULL_POLICY)
    ...
    args = ["curl --request GET http://157.245.31.108 --max-time 3"]  # github-proxy.opensafely.org
    storage = []
    create_k8s_job(job_blocked, namespace, image, command, args, {}, storage, deny_all_network_labels, image_pull_policy=IMAGE_PULL_POLICY)
